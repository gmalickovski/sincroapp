...
import React, { useState, useEffect, useRef } from 'react'; // React já está importado
...
const JournalEntryCard = React.memo(({ entry, setEditingEntry, handleDelete, onInfoClick }) => {
    const energyClasses = { 1: { border: 'border-red-500', bg: 'bg-red-500/20', text: 'text-red-300' }, 2: { border: 'border-orange-500', bg: 'bg-orange-500/20', text: 'text-orange-300' }, 3: { border: 'border-yellow-500', bg: 'bg-yellow-500/20', text: 'text-yellow-300' }, 4: { border: 'border-lime-500', bg: 'bg-lime-500/20', text: 'text-lime-300' }, 5: { border: 'border-cyan-500', bg: 'bg-cyan-500/20', text: 'text-cyan-300' }, 6: { border: 'border-blue-500', bg: 'bg-blue-500/20', text: 'text-blue-300' }, 7: { border: 'border-purple-500', bg: 'bg-purple-500/20', text: 'text-purple-300' }, 8: { border: 'border-pink-500', bg: 'bg-pink-500/20', text: 'text-pink-300' }, 9: { border: 'border-teal-500', bg: 'bg-teal-500/20', text: 'text-teal-300' }, 11: { border: 'border-violet-400', bg: 'bg-violet-400/20', text: 'text-violet-300' }, 22: { border: 'border-indigo-400', bg: 'bg-indigo-400/20', text: 'text-indigo-300' }, default: { border: 'border-gray-700', bg: 'bg-gray-800/50', text: 'text-gray-300' } };
    ...
    // Para otimização, evitamos re-renderizações desnecessárias
    console.log(`Renderizando JournalEntryCard para: ${formattedDate}`);

    return (
        <div className="grid grid-cols-[auto_1fr] gap-4 items-start">
            <div className="w-4 h-full flex flex-col items-center relative">
                <div className="absolute top-1 w-3 h-3 bg-gray-600 rounded-full"></div>
                <div className="h-full w-px bg-gray-700 mt-1"></div>
            </div>
            <div className="min-w-0 pt-px">
                 <div onClick={() => setEditingEntry(entry)} className={`w-full bg-gray-800/60 border ${currentEnergy.border} rounded-xl shadow-lg animate-fade-in group relative hover:border-purple-400/80 cursor-pointer transition-all duration-300 flex flex-col`}>
                    ...
                    <div className="p-4"><p className="text-gray-300 whitespace-pre-wrap break-word text-sm line-clamp-5 font-serif">{entry.content}</p></div>
                </div>
            </div>
        </div>
    );
...
// O restante do seu arquivo Journal.jsx permanece exatamente igual...
// ...
// ...
...
                        </button>
                    </div> : 
                    (<div className="space-y-6">{entries.map(entry => (<JournalEntryCard key={entry.id} entry={entry} setEditingEntry={setEditingEntry} handleDelete={handleDeleteNote} onInfoClick={onInfoClick}/>))}</div>)
                }
