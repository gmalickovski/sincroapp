            ...
            <div className="space-y-2">
                {tasks.map((task, index) => (
                    <div key={task.id} className="flex items-center group">
                        ...
                        <div className="flex-shrink-0 ml-2 w-8 h-8 flex items-center justify-center sm:opacity-0 group-hover:sm:opacity-100 transition-opacity">
                            <button onClick={() => onDeleteTask(task.id)} className="p-1 rounded-md text-gray-600 hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-purple-500" title="Excluir tarefa"><TrashIcon className="h-5 w-5" /></button>
                        </div>
                    </div>
                ))}
...
const Tasks = ({ userData }) => {
    ...
    const shouldShowTodayCard = !sortedDatesWithTasks.includes(todayKey);
    return (
        <>
            ...
            <div className="p-4 md:p-8 text-white max-w-3xl mx-auto w-full">
                ...
                {isLoading ? <div className="flex justify-center mt-16"><Spinner /></div> : (
                    <div className="space-y-8">
                        ...
                        {shouldShowTodayCard && (
                             <TaskCard key={todayKey} cardRef={todayCardRef} date={todayKey} tasks={[]} cardStatus={'today'} personalDay={numerologyEngine.calculatePersonalDayForDate(new Date(), userData.dataNasc)} onAddTask={handleAddTask} onToggleTask={() => {}} onDeleteTask={() => {}} onUpdateTask={() => {}} onDeleteCard={() => {}} />
                        )}
